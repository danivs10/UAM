...
int node_getConnect (const Node * n){
  if (!n){
    fprintf (stdout, "Value of errno getconnect: %s\n", strerror(errno));
    return -1;
  }
  return n->nConnect;
}
...
Status node_setNConnect (Node *n, const int cn){
  if (!n || cn<0){
    fprintf (stdout, "Value of errno setnconnect: %s\n", strerror(errno));
    return ERROR;
  }
  n->nConnect=cn;
  return OK;
...
void * node_copy (const void *src){
  ...
  node_setPredecessorId(trg, node_getPredecessorId(n));
  return trg;
}
...
int node_print (FILE *pf, const void *n){
  Node* aux;
  if(!n || !pf) {
    fprintf (stderr, "Value of errno for nodeprint: %s\n", strerror(errno));
    return -1;
  }
  aux = (Node*)n;
  return fprintf(pf, "[%ld,  %s, %ld, %d, %d] ", node_getId(aux), node_getName(aux), node_getPredecessorId(aux), node_getLabel(aux), node_getConnect(aux));
}