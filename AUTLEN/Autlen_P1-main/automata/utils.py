...
def deterministic_automata_isomorphism(
    ...
    if set(automaton1.symbols) != set(automaton2.symbols):
        #print(str(automaton1.symbols) + " --  IS NOT  --" + str(automaton2.symbols))
        return None

    if len(automaton1.states) != len(automaton2.states):
        #print(str(len(automaton1.states)) + " --  IS NOT S --" + str(len(automaton2.states)))
        return None

    if len(automaton1.transitions) != len(automaton2.transitions):
        #print(str(len(automaton1.transitions)) + " --  IS NOT T --" + str(len(automaton2.transitions)))
        return None
    ...
    while pending:
        ...
        #print(str(state1.name) + " <-- estado 1 :: estado 2 --> " +str(state2.name))
        if state1.is_final is not state2.is_final:
            #print("state 1 is final: " + str(state1.is_final) + "\n and state 2 is final: " + str(state2.is_final))
            return None
        ...
        if equiv_state:
            ...
            if len(transitions1) != len(transitions2):
                #print(str(transitions1.items()) + " --  IS NOT T1T2 --" + str(transitions2.items()))
                return None
