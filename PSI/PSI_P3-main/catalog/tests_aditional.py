from .views import SearchResultsView, book_detail, BookListView
...
class AditionalTestsModels(TestCase):
    ...
    # Test to check the get_context_data method in book_detail class
    def test10_book_detail_get_context_data(self):
        "Test get_context_data in book_detail"
        book = self.create_check(self.bookDict, Book)
        book.save()
        request = RequestFactory().get('/catalog/' + str(book.slug))
        response = book_detail.as_view()(request, slug=book.slug)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context_data['object'].pk, book.pk)
