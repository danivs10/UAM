...
def index(request):
    ...
    # Available books (status = 'a')
    # num_books_containing = Book.objects.all().count()
    coincidence = "a"
    num_books_containing = \
        Book.objects.filter(title__icontains=coincidence).count()
    num_genres_containing = \
...
def renew_book_librarian(request, pk):
    """View function for renewing a specific
    BookInstance by librarian."""
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # as required (here we just write it to
            # the model due_back field)
            book_instance.due_back = form.\
...
class AuthorUpdate(PermissionRequiredMixin, UpdateView):
    ...
    model = Author
    fields = '__all__'
    # Not recommended (potential security
...
class BookUpdate(PermissionRequiredMixin, UpdateView):
    ...
    model = Book
    fields = '__all__'
    # Not recommended (potential security issue
    # if more fields added)
