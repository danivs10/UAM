...
int main(int nargs, char **argv)
{
    ...
    for (int file_i = 1; file_i < nargs; file_i++)
    {
        ...
        // RGB to grey scale
        int r, g, b;
        #pragma omp parallel for shared(grey_image) private(r, g, b)
...
#define PIXEL_EDGES(x, y) (edges[(x) + (y)*width_edges]);
		...
		#pragma omp parallel for shared(edges_denoised) private(x, y, k, array)
        ...
        // More classic gaussian filter
            ...
            double sum = 0;
            #pragma omp parallel for shared(edges_denoised) firstprivate(sum) private(x, y)
            for (int i = radius; i < width_edges - radius; i++)
            {
                for (int j = radius; j < height_edges - radius; j++)
                {
